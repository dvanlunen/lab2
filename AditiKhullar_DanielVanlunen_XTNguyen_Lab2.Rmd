...
```
##Looking at the means of the explanatory variables across shelves
```{r fig.width=8, fig.height=9, warning=FALSE, message=FALSE}
attach(mtcars)
par(mfrow=c(3,1))
library(gplots)
# Plot the mean of teeth length by dose groups
plotmeans(sugar  ~ Shelf_factor, data = d, frame = FALSE, xlab = "Shelfs", main = "Plots of means of explanatory variablse with 95% CI", lwd=2, col=1, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
plotmeans(sodium  ~ Shelf_factor, data = d, frame = FALSE, xlab = "Shelfs", lwd=2, col = 2, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
plotmeans(fat  ~ Shelf_factor, data = d, frame = FALSE, xlab = "Shelfs", lwd=2, col = 3, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)

```

* Shelf 1 appears to have highest sodium content and small serving sizes. Sodium content is decreasing as we go up the shelfs, expect for shelf 4, where the mean is slightly higher than shelf 3.


* Shelf 2 has high sugar(highest mean sugar content) and small serving sizes. 
* Shelf 3 and 4 have large serving sizes. 
...
```{r}
# Check each two-way intercation for significance
car::Anova(nnet::multinom(
