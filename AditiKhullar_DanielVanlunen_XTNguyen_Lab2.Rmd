...
library(ggplot2)
library(MASS) #used to make ggplot parallel coordinates plots
library(dplyr)
```
...
Based on the LRT test results aboe, we can conclude there are no significant interactions among the explanatory variables (including an interaction among all three variables) since for all interaction effects, p values are greater than 0.05.
...
This is consistent with our exploratory analysis in part (b), where shelf 2 appears t have the highest sugar content, and shelves 3 and 4 have similar sugar content as the probability of being on 3 or 4 follows each other closely as the sugar content increases. Also, shelf 1 has a wide range of sugar content, we can see that from this plot where, for different levels of sodium, we can see the curve for shelf 1 differ greatly. This indicates that, shelf 1 has a wide range of products, both high sugar and sodium, and low sugar and sodium.
